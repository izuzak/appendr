...
* [Running your own version on AppEngine](#running-your-own-version-on-appengine)
* [Credits](#credits)
* [License](#license)
...
* **does not store any data** except for the connection to an external storage service. The data you collect is still stored on services that you trust and you can always modify that data using those services.

* **simplifies the process of automated data collection** if this process is append-only oriented. Most data storage services have APIs to fetch and store data, but not to just append data with a single API call. Consequently, you have to do a fetch-modify-store cycle every time you want to append data. Appendr wraps APIs of external storage services in order to provide a simple API for timestamping and appending data.
...
0. **Creating [OAuth](http://en.wikipedia.org/wiki/OAuth) tokens for external storage services you want to use**. This step must be done by visiting the [Appendr website](https://appendr.appspot.com/#tokens) in your browser, but it is generally enough to do it only once. However, you can create the tokens again, as many times as you want, if you forget them.

1. **Creating an Appendr bin** by choosing an external storage service and providing an OAuth token for it. You can create as many bins as you want. A bin is a unique link to a specific external storage service - you can think of it as a file which is stored externally, but you have additional permission to append to it via Appendr. Bins can be created either via a form on [Appendr's website](https://appendr.appspot.com/#create) or via [Appendr's API](#create-a-bin). Creating a bin gives you links to raw and HTML versions of the associated data stored on the external storage service used for that bin.

2. **Sending key-value data to a bin**. Appendr automatically timestamps data sent to a bin (as a new key-value pair in the dataset) and then appends this data to the associated file in the external storage. This step must be done via [Appendr's API](#append-data). You can also use a bin's web page to append some dummy data. Each bin keeps track of the status of data append tasks and exposes these statuses when you query the bin.

3. **Get a list of your bins** stored on a specific external storage service by providing an OAuth token for that service. Again, this can be done either via a form on [Appendr's website](https://appendr.appspot.com/#search) or via [Appendr's API](#find-bins).
...
1) As is the whole application, the API is accessible only via HTTPS.
...
5) In order to get JSON responses from the API, you have to specify an `Accept` HTTP header in the request that defines `application/json` as the desired response format: e.g. `Accept: application/json`.

6) In case an error happens during processing, you will receive a response with a JSON object with the following properties:
...
* `details` - a human-readable string with a description of the error that occurred.
...
Example response (after creating a Gist with a file named `data.json`):
...
See [Get a bin](#get-a-bin) section below for an explanation of the properties of the returned JSON object.
...
* key-value pair dataset that will be timestamped and appended to the existing data in external storage
...
Example response:
...

    {
      ...
      "task_url": "https://appendr.appspot.com/bins/123abc456def789ghi00/tasks/foobarbazboom",
      "tasks_url": "https://appendr.appspot.com/bins/123abc456def789ghi00/tasks",
      "status": "completed",
      "status_msg": "",
...
See [Get a task](#get-a-task) section below for an explanation of the properties of the returned JSON object.
...
Created by [Ivan Zuzak](http://ivanzuzak.info). Licensed under the [Apache 2.0 License](https://github.com/izuzak/appendr/blob/master/LICENSE).
